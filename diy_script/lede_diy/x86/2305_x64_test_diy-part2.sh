...
sed -i 's/LEDE/OpenWrt_2305_x64_精简版 by CAIYA build/g' package/lean/default-settings/files/zzz-default-settings
echo "zzz-default-settings文件替换完成："
echo "增加编译时间后的zzz-default-settings文件内容为："
cat package/lean/default-settings/files/zzz-default-settings

# 修改右下角脚本版本信息
sed -i 's/<a class=\"luci-link\" href=\"https:\/\/github.com\/openwrt\/luci\" target=\"_blank\">Powered by <%= ver.luciname %> (<%= ver.luciversion %>)<\/a>/OpenWrt_2305_x64_测试版 by CAIYA build @R'"$build_date"'/' package/luci-theme-argon/luasrc/view/themes/argon/footer.htm
...
sed -i "/<a class=\"luci-link\"/d; /<a href=\"https:\/\/github.com\/jerrykuku\/luci-theme-argon\"/d; s|<%= ver.distversion %>|OpenWrt_2305_x64_测试版 by CAIYA build @R$build_date|" package/luci-theme-argon/luasrc/view/themes/argon/footer_login.htm
